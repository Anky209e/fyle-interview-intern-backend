...
def upsert_assignment(p, incoming_payload):
    ...
    if incoming_payload.get('content') is None:
        raise FyleError(400, 'Cannot Edit with Null')
...
def submit_assignment(p, incoming_payload):
    ...
    cur_assignment = Assignment.get_by_id(incoming_payload.get('id'))

    if cur_assignment.state == AssignmentStateEnum.SUBMITTED or cur_assignment.state == AssignmentStateEnum.GRADED:
